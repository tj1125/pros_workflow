"""Publish Approach Agent arm motions through tools/car_control.
...
each point as a structured ``arm_control_signal`` command.  tools/car_control
remains the only owner of the final ``/robot_arm`` publisher.
"""
...
def _validate_control_topic(topic: str | None) -> str:
    ...
    if topic in {DEFAULT_ARM_TOPIC, DEFAULT_ARM_TOPIC.lstrip("/")}:
        raise ValueError(
            ...
            f"{DEFAULT_ARM_TOPIC} is published by tools/car_control."
        )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "commands for tools/car_control to execute on /robot_arm."
        )
    )
