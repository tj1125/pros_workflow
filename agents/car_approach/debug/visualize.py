...
VLM_RL_ROOT = CAR_APPROACH_DIR.parents[1]
DEFAULT_OUTPUT_DIR = DEBUG_DIR / "outputs"
DEFAULT_CONFIG_PATH = CAR_APPROACH_DIR / "configs" / "car_approach.yaml"
DEFAULT_ROS_MAP_YAML = VLM_RL_ROOT / "tools" / "car_control" / "src" / "nav_goal_bridge_pkg" / "config" / "keepout_map.yaml"

if str(VLM_RL_ROOT) not in sys.path:
    sys.path.insert(0, str(VLM_RL_ROOT))
...
def _install_lightweight_car_approach_package() -> None:
    ...
    if agents_pkg is None:
        ...
        agents_pkg.__path__ = [str(VLM_RL_ROOT / "agents")]
...
def _latest_session_sqlite() -> Path | None:
    sessions_dir = VLM_RL_ROOT / "logs" / "sessions"
