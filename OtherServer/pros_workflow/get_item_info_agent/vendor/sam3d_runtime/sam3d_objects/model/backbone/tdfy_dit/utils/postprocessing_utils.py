...
def _fill_holes(
    ...
    """
    if _meshfix is None or igraph is None:
        raise ImportError(
            "fill_holes requires pymeshfix and igraph, which are not installed."
        )
...
def postprocess_mesh(
    ...
    if simplify and simplify_ratio > 0:
        if pv is None:
            if verbose:
                tqdm.write("Skipping simplify: pyvista is not installed.")
        else:
            mesh = pv.PolyData(
...
def parametrize_mesh(vertices: np.array, faces: np.array):
    ...
    if xatlas is None:
        raise ImportError("parametrize_mesh requires xatlas, which is not installed.")
