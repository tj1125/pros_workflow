...
* **Nav Agent (導航節點)**：負責控制機器人底盤移動至新的觀察位置。當 VLM 判斷當前視角不足時，透過導航調整機器人位置以改善觀察條件。觸發條件：當夾爪與目標物之間的障礙物太擋，以至於無法透過微調動作來解決。

* **GraspGen Agent (抓取生成節點)**：負責生成抓取位姿。Agent 會將感知資料透過 A2A 協議傳送至 RTX 3090 推論伺服器，由 grasp generation 模型進行推論並回傳抓取位姿與相關資訊。觸發條件：當夾爪與目標物之間無障礙物時。

* **Approach Agent (靠近節點)**：負責引導夾爪接近目標物。透過向推論伺服器請求控制策略推論結果，逐步調整機械手臂位置以接近預抓取點。觸發條件：當GraspGen Agent決定好抓取位姿後，此為最後一個步驟。

* **View Agent (視野調整節點)**：負責調整機械手臂或機器人姿態以改善觀察視角，使系統能獲得更清晰的感知資訊，提升後續抓取成功率。觸發條件：當夾爪與目標物之間有障礙物，且為稍微阻擋，透過微調動作可解決。
