"""Experiment A0 execution recorder.
...
gets the next ``A0_NNN`` id). The report captures the metrics needed to evaluate
...
def _resolve_output_dir() -> Path:
    configured = os.getenv("EXPERIMENT_A0_RESULT_DIR", "").strip()
    if not configured:
...
class ExperimentRecorder:
    ...
    def __init__(
        ...
        *,
        ...
        self.experiment_prefix = experiment_prefix or os.getenv("EXPERIMENT_ID_PREFIX", "A0")
