...
class SessionMemoryStore:
    ...
    def __init__(
        ...
        *,
        ...
        # Experiment A0 execution recorder. Each pick task
        # (get_item_info_no_sam3d_node → END) is appended to the shared result file
        # <workflow>/result/result.json. The recorder resolves that location itself
        # (env EXPERIMENT_A0_RESULT_DIR overrides it), so no output_dir is forced.
        self.experiment_recorder = ExperimentRecorder(context_id=context_id)
    ...
    def record_event(self, *, step: int, node_name: str, state_update: Dict[str, Any]) -> None:
        ...
        # Feed the merged state to the A0 experiment recorder (raw update carries
        # last_execution timing; current_state carries cumulative fields).
        self.experiment_recorder.observe(node_name, state_update, self.current_state)

    def finalize_experiment(self) -> Optional[Dict[str, Any]]:
        """Flush the in-progress experiment report (safety net for interrupted runs)."""
        return self.experiment_recorder.finalize()
