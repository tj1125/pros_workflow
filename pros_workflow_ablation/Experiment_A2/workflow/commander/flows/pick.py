...
from ..ablation import collapse_to_nearest_goal, single_nearest_goal_enabled
...
class PickFlowMixin:
    ...
    async def _get_item_info_no_sam3d_node(self, state: CommanderState) -> Dict[str, Any]:
        ...
        center_frame = str(payload.get("center_world_coordinate_frame", "unity_world"))
        # Ablation A2: keep only the single goal pose closest to the target and drop
        # the rest of the ranked candidates. Perception is unchanged; only the
        # workflow's multi-candidate + ranking capability is removed.
        ablation_meta: Dict[str, Any] = {}
        nav_goal_pose_source = "rank_best"
        if single_nearest_goal_enabled() and group_ranking:
            target_xy = self._target_center_world_to_map_xy(
                {"center_world": center_world, "center_world_coordinate_frame": center_frame}
            )
            group_ranking, ablation_meta = collapse_to_nearest_goal(group_ranking, target_xy)
            nav_goal_pose_source = "ablation_nearest_goal"
        item_info = ItemInfoResult(
            center_world=center_world,
            center_world_coordinate_frame=center_frame,
        ...
        navigation = NavigationState(
            ...
            nav_goal_pose_source=nav_goal_pose_source,
