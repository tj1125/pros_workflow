...
Experiment A3 (``w/o get_item_info_no_sam3d_node``): the graph topology and
routing are unchanged, but ``get_item_info_no_sam3d_node`` no longer calls the
item-info A2A service — it derives candidate goal poses from the target centre and
the ROS keepout map (8 directions, push 0.4 m out per direction, keep the points
that fall in the walkable area). It is drawn as the ablated node (dashed red)
instead of an A2A node (orange), and the figure carries a title/legend describing
the ablation.
"""
...
# A3 ablation: get_item_info_no_sam3d_node no longer performs an A2A round-trip
# (it synthesises fixed-distance candidate goal poses locally), so it is no
# longer an A2A node — it is the ablated node instead (see ABLATED_NODES).
A2A_NODES = {
    "car_grasp_node",
}
...
def build_dot() -> str:
    lines = [
        ...
        ' labelloc="t", fontname="DejaVu Sans", fontsize="14",'
        ' label="Experiment A3 ablation — w/o get_item_info_no_sam3d_node'
        '\\n(object-info estimation removed: no object size / yaw / grasp-direction grouping / ranked goal poses;'
        '\\ncandidate goal poses = target center pushed 0.4 m out in 8 directions, keep the walkable ones on the keepout map)'
        '\\nLegend: orange = A2A service node;   dashed red = ablated in A3"];',
