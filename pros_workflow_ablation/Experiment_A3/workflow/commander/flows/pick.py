...
class PickFlowMixin:
    ...
    @staticmethod
    def _ablation_goal_settings() -> tuple[float, int, float]:
        """Basic sector / fallback settings for the A3 ablation.

        Ablation A3 (``w/o get_item_info_no_sam3d_node``) removes object-information
        estimation: object size, object yaw, grasp-direction grouping and the
        grasp-feasibility ranked goal poses produced by the no-SAM3D item-info
        A2A service. Candidate goal poses are instead synthesised from the target
        centre and the ROS keepout map (see ``_ablation_map_settings`` and
        ``commander/nav/ablation_goals.py``).

        - ``ABLATION_GOAL_DIRECTIONS`` (default 8): number of directions / sectors
          ("8 方位"). Set to 1 for a single direction.
        - ``ABLATION_GOAL_START_DEG`` (default 0): first direction / sector angle.
        - ``ABLATION_GOAL_DISTANCE_M`` (default 0.6): fixed stand-off distance used
          by the default ``fixed_offset`` mode (push this far out from the target
          centre per direction, then keep the walkable points).
        """
        try:
            distance_m = float(os.getenv("ABLATION_GOAL_DISTANCE_M", "0.6") or 0.6)
        except (TypeError, ValueError):
            distance_m = 0.6
        try:
            directions = int(float(os.getenv("ABLATION_GOAL_DIRECTIONS", "8") or 8))
        except (TypeError, ValueError):
            directions = 8
        directions = max(1, directions)
        try:
            start_deg = float(os.getenv("ABLATION_GOAL_START_DEG", "0") or 0.0)
        except (TypeError, ValueError):
            start_deg = 0.0
        return distance_m, directions, start_deg

    @staticmethod
    def _ablation_map_settings() -> dict[str, Any]:
        """Keepout-map feasibility settings (mirrors the item-info A2A service)."""
        ...
        default_map = repo_root / "src" / "nav_goal_bridge_pkg" / "config" / "keepout_map.yaml"
        return {
            "map_yaml_path": os.getenv("ABLATION_KEEPOUT_MAP_YAML", str(default_map)),
            "robot_radius_m": _f("ABLATION_ROBOT_RADIUS_M", 0.25),
            "black_threshold": int(_f("ABLATION_BLACK_THRESHOLD", 50)),
            "r_min_m": _f("ABLATION_GOAL_R_MIN_M", 0.4),
            "r_max_m": _f("ABLATION_GOAL_R_MAX_M", 1.5),
        }

    def _save_goal_pose_overlay(self, state: CommanderState, center_world: list[Any], frame: str, group_ranking: list[dict[str, Any]]) -> None:
        """Save a keepout-map image showing only the feasible (kept) goal poses.

        Only the candidates actually used for navigation are drawn (green); the
        infeasible directions are neither shown nor listed. Feasibility is based
        solely on the robot-radius footprint circle — the heading arrow is only
        drawn for orientation and never affects feasibility.
        """
        if str(os.getenv("ABLATION_SAVE_GOAL_MAP", "1")).strip().lower() in {"0", "false", "no", "off"}:
            return
        try:
            from ..nav.ablation_goals import save_goal_pose_overlay

            target_map_x, target_map_y = self._target_center_world_to_map_xy(
                {"center_world": center_world, "center_world_coordinate_frame": frame}
            )
            if target_map_x is None or target_map_y is None:
                return
            settings = self._ablation_map_settings()
            instance_key = str((state.get("selected_instance", {}) or {}).get("instance_key", "") or "target")
            default_dir = Path(__file__).resolve().parents[2] / "result"
            out_dir = os.getenv("ABLATION_GOAL_MAP_DIR", str(default_dir))
            out_path = str(Path(out_dir) / "goal_pose_maps.png")
            mode = str(group_ranking[0].get("selection_mode", "")) if group_ranking else "ablation_fixed_offset"
            title = f"A3 {mode}  feasible goals={len(group_ranking)}  {instance_key}"
            saved = save_goal_pose_overlay(
            ...
            if saved:
                logger.info("[get_item_info_no_sam3d_node] goal-pose overlay saved: %s", saved)
        except Exception as exc:
            logger.warning("[get_item_info_no_sam3d_node] failed to save goal-pose overlay: %s", exc)

    def _ablation_group_ranking(self, center_world: list[Any], frame: str) -> tuple[list[dict[str, Any]], str]:
        """Build candidate goal poses for the A3 ablation (no item info).

        Mode is chosen by ``ABLATION_GOAL_MODE`` (default ``fixed_offset``):

        - ``fixed_offset``: push ``ABLATION_GOAL_DISTANCE_M`` (default 0.6 m) out
          from the target centre in each of the 8 directions, then keep only the
          points with no obstacle in the footprint. A goal is feasible when no
          black/occupied cell (``ABLATION_BLACK_THRESHOLD``) lies within
          ``ABLATION_ROBOT_RADIUS_M`` (default 0.25 m) of it; white and gray cells
          are allowed. Non-feasible directions are dropped — infeasible goal poses
          are never kept.
        - ``sector_centroid``: keep walkable sectors and place the goal at the
          centroid of each sector's walkable cells.

        If no direction is walkable the fixed-offset path returns nothing (the node
        fails to NO_VALID_GOAL); if the keepout map is unavailable a plain
        fixed-distance ring is used. The heading is filled in later by
        ``goal_pose_from_ros_map`` (face the target centre); ``rank`` is just the
        fixed geometric order, not a grasp-quality ranking.
        """
        target_map_x, target_map_y = self._target_center_world_to_map_xy(
            {"center_world": center_world, "center_world_coordinate_frame": frame}
        )
        if target_map_x is None or target_map_y is None:
            return [], "could not project target center to ros map xy"

        distance_m, directions, start_deg = self._ablation_goal_settings()
        mode = str(os.getenv("ABLATION_GOAL_MODE", "fixed_offset") or "fixed_offset").strip().lower()

        if mode == "sector_centroid":
            sector = self._ablation_sector_centroid_candidates(target_map_x, target_map_y, directions, start_deg)
            if sector:
                group_ranking = []
                for rank, candidate in enumerate(sector, 1):
                    goal = candidate["goal_pose_ros_map"]
                    group_ranking.append(
                        {
                            "rank": rank,
                            "orientation_group": None,
                            "best_confidence": None,
                            "best_goal_pose_ros_map": [float(goal[0]), float(goal[1])],
                            "map_feasible": True,
                            "selection_mode": "ablation_sector_centroid",
                            "sector_index": int(candidate["sector_index"]),
                            "sector_center_deg": float(candidate["sector_center_deg"]),
                            "sector_cell_count": int(candidate["sector_cell_count"]),
                            "mean_distance_m": float(candidate["mean_distance_m"]),
                        }
                    )
                return group_ranking, ""
            logger.warning("[get_item_info_no_sam3d_node] no walkable sector; falling back to fixed-offset")

        offset_group = self._ablation_fixed_offset_group_ranking(target_map_x, target_map_y, distance_m, directions, start_deg)
        if offset_group:
            return offset_group, ""

        # No feasible goal pose (every direction's footprint hits an obstacle, or
        # the keepout map is unavailable). Do NOT fall back to unchecked points —
        # return empty so the node fails to NO_VALID_GOAL and the graph routes to
        # nav_home -> END.
        return [], f"no feasible goal pose at {distance_m:.2f}m: all {directions} directions blocked by obstacles"

    def _ablation_fixed_offset_group_ranking(
        self, target_map_x: float, target_map_y: float, distance_m: float, directions: int, start_deg: float
    ) -> list[dict[str, Any]]:
        """Center + distance_m per direction; keep only feasible points (footprint
        circle has no obstacle), drop the rest. Returns [] when nothing is feasible
        (every direction blocked, or the keepout map could not be read) — the caller
        then fails to NO_VALID_GOAL instead of producing unrunnable points.
        """
        from ..nav.ablation_goals import fixed_offset_feasible_candidates

        settings = self._ablation_map_settings()

        def _generate(keep_infeasible: bool) -> list[dict[str, Any]]:
            try:
                return fixed_offset_feasible_candidates(
                    target_map_x,
                    target_map_y,
                    map_yaml_path=settings["map_yaml_path"],
                    robot_radius_m=settings["robot_radius_m"],
                ...
                logger.warning("[get_item_info_no_sam3d_node] fixed-offset generation failed: %s", exc)
                return []

        # Only keep walkable goal poses. Infeasible directions can't be navigated
        # to, so they are dropped entirely (no infeasible fallback); if none are
        # walkable we return [] and the node fails to NO_VALID_GOAL.
        candidates = _generate(keep_infeasible=False)
        if not candidates:
            return []
        selection_mode = "ablation_fixed_offset"
            ...
            group_ranking.append(
                {
                    ...
                    "best_confidence": None,
                    "best_goal_pose_ros_map": [float(goal[0]), float(goal[1])],
                    "map_feasible": bool(candidate["map_feasible"]),
                    "selection_mode": selection_mode,
                    "approach_distance_m": float(candidate["distance_m"]),
                    "approach_direction_deg": float(candidate["direction_deg"]),
                }
            )
        return group_ranking

    def _ablation_sector_centroid_candidates(
        self, target_map_x: float, target_map_y: float, num_sectors: int, start_deg: float
    ) -> list[dict[str, Any]]:
        from ..nav.ablation_goals import sector_centroid_candidates

        settings = self._ablation_map_settings()
        try:
            return sector_centroid_candidates(
                target_map_x,
                target_map_y,
                map_yaml_path=settings["map_yaml_path"],
                robot_radius_m=settings["robot_radius_m"],
                black_threshold=settings["black_threshold"],
                r_min_m=settings["r_min_m"],
                r_max_m=settings["r_max_m"],
                num_sectors=num_sectors,
                start_deg=start_deg,
            )
        except Exception as exc:
            logger.warning("[get_item_info_no_sam3d_node] sector-centroid generation failed: %s", exc)
            return []

    async def _get_item_info_no_sam3d_node(self, state: CommanderState) -> Dict[str, Any]:
        # Ablation A3: object-information estimation is removed. The full system
        # calls the no-SAM3D item-info A2A service to estimate object size, yaw,
        # grasp-direction grouping and grasp-feasibility ranked goal poses. Here we
        # skip that service (and its room-camera capture) entirely and synthesise
        # candidate goal poses from the target centre plus the ROS keepout map:
        # push a fixed stand-off out in 8 directions and keep only the ones whose
        # robot-radius footprint has no obstacle; if none are feasible the node
        # fails (NO_VALID_GOAL) and the graph routes to END (see
        # ``_ablation_group_ranking`` / ``commander/nav/ablation_goals.py``). The
        # node keeps its name and output contract so the rest of the graph and the
        # Experiment_A3 report window (get_item_info_no_sam3d_node -> END) are
        # unchanged.
        started = time.time()
        ...
        center_world = [float(v) for v in (selected.get("center_world", []) or [])]
        frame = "unity_world"
        group_ranking, gerr = self._ablation_group_ranking(center_world, frame)
        # Always save the diagnostic overlay (all 8 directions with their real
        # per-direction feasibility), even when nothing is feasible, so the map
        # always shows why each direction was kept (green) or dropped (orange).
        self._save_goal_pose_overlay(state, center_world, frame, group_ranking)
        if not group_ranking:
            status = "ITEM_INFO_NO_SAM3D_FAILED"
            return {"item_info": dump_model(ItemInfoResult()), "current_status": status, "last_execution": self._execution(state, "get_item_info_no_sam3d_node", status, started, success=False, error=gerr or "no feasible candidate goal pose")}

        item_info = ItemInfoResult(
            center_world=center_world,
            center_world_coordinate_frame=frame,
            primary_camera_id=str(selected.get("primary_camera", "")),
            target_instance_key=str(selected.get("instance_key", "")),
            target_topic_key=str(selected.get("topic_key", "")),
            group_ranking=group_ranking,
            goal_pose_path="",
            a2a_task_id="",
        )
        ...
        mode = str(group_ranking[0].get("selection_mode", "ablation")) if group_ranking else "ablation"
        message = f"ablation goals [{mode}]: {len(group_ranking)} candidate(s)" if not err else err
        return {"item_info": dump_model(item_info), "navigation": dump_model(navigation), "current_status": status, "last_execution": self._execution(state, "get_item_info_no_sam3d_node", status, started, success=not bool(err), message=message)}
