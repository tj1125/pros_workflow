...
def check_tensor(tensor, shape, throw=True):
    ...
    shape supports None as wildcard for each dimension.
    """
    ...
    if ok:
        for i, expected in enumerate(shape):
            if expected is not None and tensor.shape[i] != expected:
                ...
                break
    if throw and not ok:
        raise ValueError(f"Tensor shape mismatch. expected={shape}, got={tuple(tensor.shape) if torch.is_tensor(tensor) else type(tensor)}")
    return ok
