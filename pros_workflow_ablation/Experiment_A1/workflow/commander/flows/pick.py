...
class PickFlowMixin:
    ...
    # removed. The fixed pick flow is expressed purely by graph edges in
    # Orchestrator._build_graph:
    #   get_item_info -> nav_move -> (world refresh) -> car_grasp -> car_approach
    #     car_approach success -> nav_home (END)
    #     car_approach fail     -> update_memory -> (world refresh) -> major_nav
    #                              -> nav_move -> ... (try the next ranked goal pose)
    async def _major_nav_node(self, state: CommanderState) -> Dict[str, Any]:
        ...
        # Advancing to a new candidate viewpoint: clear the previous viewpoint's
        # grasp/approach outcomes so each point starts a fresh attempt.
