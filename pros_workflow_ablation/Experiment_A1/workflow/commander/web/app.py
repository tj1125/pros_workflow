...
async def _stream_graph(
    ...
    # Ablation A1 execution recorder: records this stream from
    # get_item_info_no_sam3d_node to the end of the pick workflow. Chat-only turns
    # never reach that node, so report.write() is gated on report.started below.
    report = ExperimentReport(context_id=session.session_id)
    ...
    try:
        ...
        if report.started:
            ...
            if report_path:
                logger.info("[web_server] Experiment A1 report written: %s", report_path)
