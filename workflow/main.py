...
def _validate_env() -> None:
    ...
    if not os.getenv("OLLAMA_BASE_URL"):
        raise MissingConfigError(
            "OLLAMA_BASE_URL is required. Set it in .env."
        )
...
def main(
    ...
    asyncio.run(_run(max_steps=max_steps, log_path=log_path))


async def _run(max_steps: int, log_path: str) -> None:
    """Async main loop for the LangGraph orchestration."""
    logger.info("Starting VLM grasp system")
