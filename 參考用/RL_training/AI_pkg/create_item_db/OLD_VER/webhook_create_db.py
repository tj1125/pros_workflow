...
# IP = "192.168.75.62"
IP = "localhost"  # 使用 localhost 以便於本地測試
N8N_WEBHOOK_URL = "http://localhost:5678/webhook/insert-object"  # <-- 你自己的 n8n webhook URL
...
# ======== 傳送 JSON 到 n8n webhook ========
def post_to_webhook(data):
    try:
        response = requests.post(N8N_WEBHOOK_URL, json=data)
        response.raise_for_status()
        print(f"✅ 已發送資料到 n8n Webhook ({response.status_code})")
    except Exception as e:
        print(f"❌ 發送到 webhook 時失敗：{e}")
...
def handle_image(name):
    def callback(message):
        try:
            ...
            # print(f"📝 生成的 caption: {merged_caption}")
            ...
            embedding = embed_text(merged_caption)
            image_path = f"{DB_IMAGE_PREFIX}/{name}.jpg"
            coordinate_list = [coord['x'], coord['y'], coord['z']]

            data = {
                "id": None,
                "image_path": image_path,
                "content": "Coordinate: " + str(coordinate_list) + "\n" + "Caption: " + merged_caption + "\n" + "image_path: " + image_path,
                "embedding": embedding,
            }

            post_to_webhook(data)
            print(f"📸 圖片處理完成並送出: {name} -> {file_path}")
...
def start_roslibpy_client():
    ...
    if not object_names:
        print("⚠️ 無可用的物件 topic，請確認 ros publisher 是否已啟動")
